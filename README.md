...
All actions are reachable from the IDE's top-level **Refactor** menu **and** from the editor's right-click menu: the plugin takes over the IDE's own **Refactoring** entry in both places (same position, same localized caption, the IDE's own entries hidden) and fills it with its own, context-sensitively enabled entries. If the editor popup has no Refactoring entry to take over, a **Refactoring Light** submenu is appended at the end instead.

...
| *(automatic + menu)*   | **Auto-import missing units (live quick fix)** &mdash; a VS-style lightbulb appears at the caret whenever the line contains an undeclared identifier whose declaring unit the index knows; one click adds the `uses` entry. Also reachable on demand via *Add unit for identifier at cursor* / *Resolve missing units...* (batch dialog), and by double-clicking an `E2003` entry in the IDE's Structure view |
...
### Auto-import missing units (live quick fix)

The "add the missing `uses` entry" workflow, driven by real compiler diagnostics instead of guessing: an undeclared identifier is reported as error **E2003**, the identifier is taken from the reported source range (never from the localized message text), looked up in the background unit index, and the declaring unit is offered for insertion.

- **Live lightbulb**: whenever the caret sits on a line with an E2003 whose identifier the index can resolve, a small non-focus-stealing hint appears just below the caret (&#128161; *Add unit for "TStringList"*). Clicking it opens a chooser popup anchored at the caret: candidate unit(s), target section (`interface` / `implementation`), *Add* button; double-click on a unit inserts immediately, Escape / clicking elsewhere cancels. The hint is bound to the **error line** &mdash; it hides when the caret moves elsewhere, the line scrolls out of view, or the buffer changes.
- **Two diagnostics sources**:
  - **In the IDE** the primary source is the IDE's own Error Insight, tapped via the official Structure-view API (`IOTAStructureView` + `IOTAStructureNotifier`): the Structure pane mirrors the errors the IDE's internal DelphiLSP produces, and `StructureChanged` fires live on every re-evaluation &mdash; so the lightbulb appears in step with the IDE's red squiggle, with **no LSP session of our own** involved and no need for a saved project.
  - **Fallback (and the standalone exe)**: a low-frequency poller watches the active buffer; after ~1.5 s of idle time it pushes the (unsaved) buffer to the plugin's own DelphiLSP session in the background and collects the E2003 diagnostics. In the IDE the fallback stays quiet for any buffer state the Structure view already answered; it takes over when Error Insight is disabled. It never cold-starts the LSP by itself.
- **Structure-view double-click**: double-clicking an E2003 entry in the Structure pane's *Errors* node performs the IDE's normal jump-to-error and then opens the quick-fix popup right at the error position.

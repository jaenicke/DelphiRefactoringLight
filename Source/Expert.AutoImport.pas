...
/// <summary>True when the statement starting on line ALine0 is the SOLE
///  statement of a control structure: the previous code token (comments
///  skipped, also multi-line ones) is THEN, ELSE, DO or a label/case
///  colon. Deleting such a statement would make the FOLLOWING statement
///  conditional ("if not FCached then" / "  FRetries := 0;" / "Load;").</summary>
function IsSoleBranchStatement(const ALines: TArray<string>; ALine0: Integer): Boolean;

/// <summary>The body line of a generated implementation stub (E2065 /
///  DFM handler): '' for ordinary routines; for an instance CONSTRUCTOR or
///  DESTRUCTOR the inherited call - a constructor stub without it compiles
///  and fails much later, far from the cause. 'override' or a destructor ->
///  'inherited;'; a new constructor of a TObject descendant ->
///  'inherited Create;'; any other parent -> a TODO comment (the right
///  ancestor constructor cannot be guessed); records -> nothing.
///  ADeclText = the declaration's text incl. directives, AOpenerLine = the
///  enclosing type's "TFoo = class(TBar)" line.</summary>
function StubBodyLine(const AKind: string; AIsClassMethod: Boolean;
  const ADeclText, AOpenerLine: string): string;

/// <summary>E2291 "Missing implementation of interface method IFoo.Bar":
///  the interface and method named by the (localized) message - the LAST
///  dotted identifier in it, quotes ignored. False when there is none.</summary>
function E2291MethodFromMessage(const AMessage: string; out AIntf, AMethod: string): Boolean;

/// <summary>The class-side declaration for the interface method declared
///  at ADeclLine0 of AIntfLines: "procedure Bar(const A: Integer);" plus the
///  calling convention it carries (stdcall/safecall/cdecl/pascal/register -
///  it is part of the interface contract). False for an overloaded method
///  (the diagnostic cannot say which overload is missing) or an unreadable
///  header.</summary>
function InterfaceMethodDecl(const AIntfLines: TArray<string>; ADeclLine0: Integer;
  out ADecl: string): Boolean;

/// <summary>Where ADecls go into the class declared at AClassLine0: the end
///  of its (last) PUBLIC section, or a new "public" section before the
///  class's 'end;'. AText is ready for InsertTextAtLineStart(AInsLine),
///  AFirstDeclLine the 0-based line the first declaration will occupy.
///  False when the class body cannot be delimited.</summary>
...
/// <summary>0-based line of the 'implementation' keyword, or MaxInt when none.</summary>
function ImplementationLineOf(const ALines: TArray<string>): Integer;
/// <summary>S is a routine header: procedure / function / constructor /
///  destructor, optionally with 'class '.</summary>
function IsHeaderLine(const S: string; out AKind: string;
  out AIsClassMethod: Boolean): Boolean;
/// <summary>Joins lines from AStart up to the first ';' outside brackets
///  (the signature); AEndLine = last joined line. '' when none.</summary>
function CollectHeader(const ALines: TArray<string>; AStart: Integer;
  out AEndLine: Integer): string;
/// <summary>Name (possibly qualified), parameter text and result type of a
...
/// <summary>The (possibly nested, then qualified 'TOuter.TInner') class or
///  record whose declaration body encloses ALine0; '' = none.</summary>
function EnclosingContainerName(const ALines: TArray<string>; ALine0: Integer): string;
/// <summary>AWord occurs in S as a whole word (case-insensitive).</summary>
function HasWholeWordCI(const S, AWord: string): Boolean;
/// <summary>0-based line an implementation block goes BEFORE: the
///  initialization/finalization keyword, else the final 'end.'; -1 for a
///  file without implementation section.</summary>
...
var
  ...
  begin
    ...
    if not IsIdentifier(Tok) then Exit;
  ...
  begin
    ...
    // "if X then" / "  A := 0;": deleting the line would pull the NEXT
    // statement under the condition.
    if IsSoleBranchStatement(Lines, D.Range.Start.Line) then Exit;
  ...
  // E2291 "Missing implementation of interface method IFoo.Bar", reported
  // at the CLASS header: copy the method's declaration from the interface
  // (this unit, or the unit the index knows for it) into the class and
  // create the empty implementation. Class Completion does NOT do this.
  procedure AddE2291(const D: TLspErrorDiag);
  var
    Intf, Meth, IntfBare, ClassName, Decl, IntfContent: string;
    IntfLines: TArray<string>;
    DeclLine, P: Integer;
    F: TQuickFix;
  begin
    if (D.Range.Start.Line < 0) or (D.Range.Start.Line > High(Lines)) then Exit;
    if not E2291MethodFromMessage(D.Message, Intf, Meth) then
    begin
      Decline(D.Code, '', 'no interface method named in the message');
      Exit;
    end;
    if Seen.ContainsKey('I91|' + IntToStr(D.Range.Start.Line) + '|' + UpperCase(Intf + '.' + Meth)) then Exit;
    Seen.Add('I91|' + IntToStr(D.Range.Start.Line) + '|' + UpperCase(Intf + '.' + Meth), True);
    ClassName := ClassOpenerName(Trim(StripLineComment(Lines[D.Range.Start.Line])));
    if ClassName = '' then
    begin
    ...
    if P > 0 then IntfBare := Copy(IntfBare, P + 1, MaxInt);
    if (IntfBare = '') or (Pos('<', IntfBare) > 0) then
    begin
      Decline(D.Code, Meth, 'generic interfaces are not supported');
      Exit;
    end;
    // the interface: this unit first, then the unit(s) the index knows
    IntfLines := Lines;
    DeclLine := FindMemberDeclarationLine(AContent, IntfBare, Meth);
    if (DeclLine < 0) and (Snap <> nil) then
      for var H in Snap.Lookup(IntfBare) do
      begin
        if (H.Path = '') or not TFile.Exists(H.Path) then Continue;
        try
          IntfContent := ReadDelphiFile(H.Path);
        except
          Continue;
        end;
        DeclLine := FindMemberDeclarationLine(IntfContent, IntfBare, Meth);
        if DeclLine >= 0 then
        begin
          IntfLines := SplitContentLines(IntfContent);
          Break;
        end;
      end;
    if DeclLine < 0 then
    begin
      Decline(D.Code, Meth, Format('the declaration of %s.%s was not found', [IntfBare, Meth]));
      Exit;
    end;
    if not InterfaceMethodDecl(IntfLines, DeclLine, Decl) then
    begin
      Decline(D.Code, Meth, 'overloaded or unreadable interface method');
      Exit;
    end;
    F := Default(TQuickFix);
    F.Kind := qfImplIntfMethod;
    F.Line := D.Range.Start.Line;
    P := Pos(UpperCase(ClassName), UpperCase(Lines[F.Line]));
    if P > 0 then F.Col := P - 1;
    F.TokenLen := Length(ClassName);
    F.Identifier := IntfBare + '.' + Meth;
    F.OldUnit := ClassName;          // class name, for the "all" caption
    F.NewText := Decl;
    F.Caption := Format('Implement %s.%s in %s', [IntfBare, Meth, ClassName]);
    Res.Add(F);
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      // position - only the message tells them apart.
      if SameText(D.Code, 'E2291') then
        Key := Key + '|' + UpperCase(D.Message);
      if Seen.ContainsKey(Key) then Continue;
      Seen.Add(Key, True);
    ...
    var ByClass := TObjectDictionary<Integer, TList<Integer>>.Create([doOwnsValues]);
    try
      for var I := 0 to Res.Count - 1 do
        if Res[I].Kind = qfImplIntfMethod then
        begin
          var Idx: TList<Integer>;
          if not ByClass.TryGetValue(Res[I].Line, Idx) then
          begin
            Idx := TList<Integer>.Create;
            ByClass.Add(Res[I].Line, Idx);
          end;
          Idx.Add(I);
        end;
      for var Pair in ByClass do
        ...
        begin
          var All := Res[Pair.Value[0]];
          All.NewText := '';
          for var I in Pair.Value do
            All.NewText := All.NewText + IfThen(All.NewText <> '', #10, '') + Res[I].NewText;
          All.Identifier := '*';
          All.Caption := Format('Implement all %d missing interface methods in %s',
            [Pair.Value.Count, All.OldUnit]);
          Res.Add(All);
        end;
    finally
      ByClass.Free;
...
// AWord as a whole word in S (case-insensitive).
function HasWholeWordCI(const S, AWord: string): Boolean;
begin
  var U := UpperCase(S);
  var W := UpperCase(AWord);
  var P := Pos(W, U);
  while P > 0 do
  begin
    var BeforeOk := (P = 1) or not CharInSet(U[P - 1], ['A'..'Z', '0'..'9', '_']);
    var AfterPos := P + Length(W);
    var AfterOk := (AfterPos > Length(U)) or not CharInSet(U[AfterPos], ['A'..'Z', '0'..'9', '_']);
...
// Parameter TYPES of a header's parameter list, normalised for comparing
// overloads: 'const A, B: Integer; var S: string = ''x''' ->
// 'const:integer;const:integer;var:string;'. Names and default values do
// not distinguish overloads (and an implementation may omit the defaults).
function OverloadParamSignature(const AParams: string): string;
begin
  Result := '';
  for var G in AParams.Split([';']) do
  begin
    var S := Trim(G);
    if S = '' then Continue;
    var P := Pos('=', S);
    if P > 0 then S := Trim(Copy(S, 1, P - 1));          // default value
    var Modifier := '';
    for var M in ['const', 'var', 'out', 'constref'] do
      if StartsText(M + ' ', S) then
      begin
        Modifier := M;
        S := Trim(Copy(S, Length(M) + 1, MaxInt));
        Break;
      end;
    var TypeName := '';
    P := Pos(':', S);
    if P > 0 then
    begin
      TypeName := LowerCase(StringReplace(Trim(Copy(S, P + 1, MaxInt)), ' ', '', [rfReplaceAll]));
      S := Copy(S, 1, P - 1);
    end;
    // one entry per NAME ('A, B: Integer' = two Integer parameters)
    for var N in S.Split([',']) do
      if Trim(N) <> '' then
        Result := Result + Modifier + ':' + TypeName + ';';
  end;
end;

function IsTypeOpenerLine(const ALine: string): Boolean; forward;
...
begin
  ...
  BareName := Qualified;
  // 'overload' anywhere in the declaration's own lines (a wrapped header
  // carries its directives on its last line)
  var IsOverload := False;
  for I := ALine0 to Min(HdrEnd, High(Lines)) do
    if HasWholeWordCI(StripLineComment(Lines[I]), 'overload') then
      IsOverload := True;
  ...
  // exists (stale diagnostics / applying the same fix twice). For an
  // OVERLOADED routine the name alone says nothing - forum report: the
  // stub for 'class procedure Init(const xBoolean: Boolean); overload;'
  // was refused because 'TMyClassStatic2.Init' (the parameterless
  // overload) was already implemented. There the parameter types decide.
  var WantSig := OverloadParamSignature(Params);
    ...
    begin
      ...
      if (Hdr <> '') and ParseHeader(Hdr, ChkKind, ChkQual, ChkParams, ChkRet)
        and (SameText(ChkQual, Qualified)
          or SameText(ChkQual, ContainerName + '.' + BareName)) then
      begin
        if not IsOverload then Exit;
        if OverloadParamSignature(ChkParams) = WantSig then Exit;
      end;
    end;

  // The declaration's own lines (directives such as 'override' live on
  // the last one) and the opener of the enclosing type decide the body.
  var DeclText := '';
  for I := ALine0 to Min(HdrEnd, High(Lines)) do
    DeclText := DeclText + ' ' + StripLineComment(Lines[I]);
  var Opener := '';
  if ContainerName <> '' then
    for I := ALine0 - 1 downto 0 do
      if IsTypeOpenerLine(Lines[I]) then
      begin
        Opener := Lines[I];
        Break;
      end;
  var Body := StubBodyLine(Kind, IsCM, DeclText, Opener);
  ...
  Text := Text + ';' + sLineBreak + 'begin' + sLineBreak + Body + sLineBreak
    + 'end;' + sLineBreak + sLineBreak;

  Result := Editor.InsertTextAtLineStart(AFile, InsLine + 1, Text);
end;

function IsTypeOpenerLine(const ALine: string): Boolean;
var
  S: string;
  P: Integer;
begin
  S := UpperCase(Trim(StripLineComment(ALine)));
  P := Pos('=', S);
  if P = 0 then Exit(False);
  S := TrimLeft(Copy(S, P + 1, MaxInt));
  if S.StartsWith('PACKED ') then S := TrimLeft(Copy(S, 8, MaxInt));
  // a forward declaration ('TFoo = class;') and a metaclass ('class of')
  // open no body
  if S.StartsWith('CLASS OF') or (S = 'CLASS;') or (S = 'INTERFACE;') then Exit(False);
  Result := S.StartsWith('CLASS') or S.StartsWith('RECORD') or S.StartsWith('OBJECT');
end;
...
begin
  Result := '';
  if AIsClassMethod then Exit;   // class constructor/destructor: nothing to chain
  if not (SameText(AKind, 'constructor') or SameText(AKind, 'destructor')) then Exit;
  Up := UpperCase(AOpenerLine);
  P := Pos('=', Up);
  if P > 0 then Up := TrimLeft(Copy(Up, P + 1, MaxInt)) else Up := '';
  if Up.StartsWith('PACKED ') then Up := TrimLeft(Copy(Up, 8, MaxInt));
  if Up.StartsWith('RECORD') then Exit;   // records have no inheritance
  // 'override': the ancestor HAS this exact signature - plain 'inherited;'
  // is right. A destructor always chains (TObject.Destroy & co).
  if HasWholeWordCI(ADeclText, 'override') or SameText(AKind, 'destructor') then
    Exit('  inherited;');
  // A NEW constructor: 'inherited;' would look for the same name AND
  // parameters in the ancestor (E2008 for 'Create(A: Integer)' on TObject).
  // Only a class derived from TObject has a known right answer.
  Parents := '';
  P := Pos('(', Up);
  if P > 0 then
  begin
    Q := Pos(')', Up, P);
    if Q > P then Parents := Trim(Copy(Up, P + 1, Q - P - 1));
    P := Pos(',', Parents);
    if P > 0 then Parents := Trim(Copy(Parents, 1, P - 1));
  end;
  if (Parents = '') or (Parents = 'TOBJECT') or (Parents = 'SYSTEM.TOBJECT') then
    Result := '  inherited Create;'
  else
    Result := '  // TODO: call the inherited constructor - inherited Create(...);';
end;
...
begin
  ...
  begin
    if CharInSet(AMessage[I], ['A'..'Z', 'a'..'z', '_']) then
    begin
      S := I;
      while (I <= Length(AMessage)) and
        CharInSet(AMessage[I], ['A'..'Z', 'a'..'z', '0'..'9', '_', '.']) do Inc(I);
      Tok := Copy(AMessage, S, I - S);
      while Tok.EndsWith('.') do Delete(Tok, Length(Tok), 1);   // sentence end
      P := LastDelimiter('.', Tok);
      if (P > 1) and (P < Length(Tok)) then
      begin
        AIntf := Copy(Tok, 1, P - 1);
        AMethod := Copy(Tok, P + 1, MaxInt);
        Result := True;   // keep scanning: the LAST dotted token wins
      end;
    end
    else
      Inc(I);
...
function InterfaceMethodDecl(const AIntfLines: TArray<string>; ADeclLine0: Integer;
  out ADecl: string): Boolean;
const
  Conventions: array[0..4] of string = ('stdcall', 'safecall', 'cdecl', 'pascal', 'register');
var
  Kind, Hdr, Qual, Params, Ret, Tail, U: string;
  IsCM: Boolean;
  HdrEnd, P: Integer;
begin
  Result := False;
  ADecl := '';
  if (ADeclLine0 < 0) or (ADeclLine0 > High(AIntfLines)) then Exit;
  if not IsHeaderLine(StripLineComment(AIntfLines[ADeclLine0]), Kind, IsCM) then Exit;
  Hdr := CollectHeader(AIntfLines, ADeclLine0, HdrEnd);
  if (Hdr = '') or not ParseHeader(Hdr, Kind, Qual, Params, Ret) then Exit;
  // directives: what follows the signature's ';' on its last line
  Tail := StripLineComment(AIntfLines[HdrEnd]);
  P := Pos(';', Tail);
  if HdrEnd = ADeclLine0 then
  begin
    // the signature's ';' is the first one outside the parameter list
    var Depth := 0;
    P := 0;
    for var I := 1 to Length(Tail) do
      case Tail[I] of
        '(': Inc(Depth);
        ')': Dec(Depth);
  ...
  if P > 0 then Tail := Copy(Tail, P + 1, MaxInt) else Tail := '';
  U := UpperCase(Tail);
  if HasWholeWordCI(Tail, 'overload') then Exit;
  ADecl := Kind + ' ' + Qual;
  if Params <> '' then ADecl := ADecl + '(' + Params + ')';
  if Ret <> '' then ADecl := ADecl + ': ' + Ret;
  ADecl := ADecl + ';';
  for var C in Conventions do
    if HasWholeWordCI(Tail, C) then
      ADecl := ADecl + ' ' + C + ';';
  Result := True;
end;
...
function PlanInterfaceMethodInsertion(const ALines: TArray<string>; AClassLine0: Integer;
  ...
  function VisibilityWord(const T: string): string;
  var
    U: string;
  begin
    U := LowerCase(T);
    if U.StartsWith('strict ') then U := Trim(Copy(U, 8, MaxInt));
    for var W in ['public', 'published', 'protected', 'private', 'automated'] do
      if (U = W) then Exit(W);
    Result := '';
  end;
...
begin
  ...
  AText := '';
  AFirstDeclLine := -1;
  if (AClassLine0 < 0) or (AClassLine0 > High(ALines)) or (Length(ADecls) = 0) then Exit;
  if ClassOpenerName(Trim(StripLineComment(ALines[AClassLine0]))) = '' then Exit;
  ...
  begin
    ...
    if (ClassOpenerName(T) <> '') or
       (T.ToUpper.Contains('= RECORD') and not T.EndsWith(';')) then
      Inc(Depth)
  ...
  begin
    // end of that public section: the next visibility keyword or the end
    NextVis := EndLine;
    Depth := 1;
    for L := LastPublic + 1 to EndLine - 1 do
    begin
      T := Trim(StripLineComment(ALines[L]));
      if (ClassOpenerName(T) <> '') or
         (T.ToUpper.Contains('= RECORD') and not T.EndsWith(';')) then Inc(Depth)
      else if SameText(T, 'end;') or SameText(T, 'end') then Dec(Depth)
      else if (Depth = 1) and (VisibilityWord(T) <> '') then
      begin
        NextVis := L;
        Break;
  ...
  for var Dcl in ADecls do
    AText := AText + MemberIndent + Dcl + sLineBreak;
  Result := True;
end;

// E2291 fix: declares the missing interface methods in the class and
// creates their empty implementations. ADecls come from the resolver
// (InterfaceMethodDecl); already declared ones are skipped.
function ImplementInterfaceMethods(const AFile: string; AClassLine0: Integer;
  const ADecls: TArray<string>): Boolean;
var
  Content, ClassName, Kind, Qual, Params, Ret, Text: string;
  Lines: TArray<string>;
  Todo: TArray<string>;
  InsLine, FirstDecl: Integer;
  IsCM: Boolean;
begin
  Result := False;
  if (Editor = nil) or not ReadCurrentContent(AFile, Content) then Exit;
  Lines := SplitContentLines(Content);
  if (AClassLine0 < 0) or (AClassLine0 > High(Lines)) then Exit;
  ClassName := ClassOpenerName(Trim(StripLineComment(Lines[AClassLine0])));
  if ClassName = '' then Exit;   // stale
  Todo := nil;
  for var Dcl in ADecls do
  begin
    if not IsHeaderLine(Dcl, Kind, IsCM) or not ParseHeader(Dcl, Kind, Qual, Params, Ret) then
      Continue;
    if FindMemberDeclarationLine(Content, ClassName, Qual) >= 0 then Continue;   // done already
    Todo := Todo + [Dcl];
  end;
  if Length(Todo) = 0 then Exit;
  if not PlanInterfaceMethodInsertion(Lines, AClassLine0, Todo, InsLine, Text, FirstDecl) then
    Exit;
  if not Editor.InsertTextAtLineStart(AFile, InsLine + 1, Text) then Exit;
  // the implementations go BELOW the class, so the declaration lines stay
  for var K := 0 to High(Todo) do
    CreateImplStub(AFile, FirstDecl + K);
  Result := True;
end;
...
begin
  ...
  // Mask up to the statement's line: { } state carries across lines.
  M := MaskCommentsAndStrings(Copy(ALines, 0, ALine0));
  for L := ALine0 - 1 downto 0 do
  begin
    S := TrimRight(M[L]);
    if S = '' then Continue;
    P := Length(S);
    if S[P] = ':' then Exit(True);           // label / case branch
    if S[P] = ';' then Exit(False);
    Q := P;
    while (Q >= 1) and CharInSet(S[Q], ['A'..'Z', 'a'..'z', '0'..'9', '_']) do Dec(Q);
    if (Q >= 1) and (S[Q] = '&') then Exit(False);   // escaped identifier
  ...
  try
    ...
    for I := 0 to High(FFixes) do
      case FFixes[I].Kind of
        ...
        qfDeclareVar, qfDeclareInlineVar, qfImplIntfMethod:
          begin
...
begin
  ...
  var R := WorkAreaAt(APt);
  Left := Max(R.Left, Min(APt.X, R.Right - Width - 4));
  Top := Max(R.Top, Min(APt.Y, R.Bottom - Height - 4));
  Show;
...
begin
  ...
  if (Top < WndR.Top) or (Top < WorkAreaAt(P).Top) then Exit;
...
begin
  R := WorkAreaAt(APt);
  X := Max(R.Left, Min(APt.X, R.Right - Width - 4));
  Y := Max(R.Top, Min(APt.Y, R.Bottom - Height - 4));
...
type
  ...
  private
    ...
    FTickNo: Cardinal;        // fast ticks since start (heavy work every N)
    ...
    FBlockLastSeen: Cardinal;   // last probe that saw FBlockWnd blocking
    FBlockCls: string;
    FAnalysing: Boolean;
...
// ALine without its comments ({ }, (* *), //); string literals stay, so a
// line ending in a string is still recognised as a statement.
function StripAllLineComments(const ALine: string): string;
var
  I, N: Integer;
  InStr: Boolean;
  SB: TStringBuilder;
  ...
  try
    ...
    begin
      ...
      case ALine[I] of
        ...
        '{':
          begin
            while (I <= N) and (ALine[I] <> '}') do Inc(I);
            Inc(I);
            SB.Append(' ');
          end;
        '(':
          ...
          begin
            Inc(I, 2);
            while (I < N) and not ((ALine[I] = '*') and (ALine[I + 1] = ')')) do Inc(I);
            Inc(I, 2);
            SB.Append(' ');
          end
          else
...
function CanTakeSemicolon(const ALine: string): Boolean;
const
  // Words a statement never ends with - a ';' behind them is invalid:
  // block openers, binary operators (the expression continues on the next
  // line), section and visibility keywords.
  Openers: array[0..40] of string = (
    'BEGIN', 'THEN', 'ELSE', 'DO', 'OF', 'TRY', 'REPEAT', 'EXCEPT',
    'FINALLY', 'VAR', 'CONST', 'TYPE', 'TO', 'DOWNTO', 'IN', 'AND',
    'OR', 'NOT', 'DIV', 'MOD', 'SHL', 'SHR', 'XOR', 'AS', 'IS',
    'WHILE', 'IF', 'FOR', 'WITH', 'CASE', 'RECORD', 'PACKED',
    'PRIVATE', 'PROTECTED', 'PUBLIC', 'PUBLISHED', 'STRICT',
    'IMPLEMENTATION', 'INITIALIZATION', 'FINALIZATION', 'USES');
var
  S, LastWord, FirstWord, U: string;
  I: Integer;

  function HasWord(const AWord: string): Boolean;
  var
    P, E: Integer;
  begin
    Result := False;
    ...
    begin
      E := P + Length(AWord);
      if ((P = 1) or not CharInSet(U[P - 1], ['A'..'Z', '0'..'9', '_'])) and
         ((E > Length(U)) or not CharInSet(U[E], ['A'..'Z', '0'..'9', '_'])) then
        Exit(True);
      P := Pos(AWord, U, P + 1);
    end;
  end;
...
begin
  Result := False;
  // ALL comments go - "if X then { TODO }" used to end in '}', the
  // last-word scan found nothing, and the guard let it through.
  S := TrimRight(StripAllLineComments(ALine));
  if S = '' then Exit;
  U := UpperCase(S);
  // A control-structure HEADER wrapped before its keyword ("for I := 0 to
  // 10" / "do" on the next line) ends in an ordinary operand, so the
  // last-word rule cannot see it - the FIRST word can.
  I := 1;
  while (I <= Length(S)) and CharInSet(S[I], [' ', #9]) do Inc(I);
  FirstWord := '';
  while (I <= Length(S)) and CharInSet(S[I], ['A'..'Z', 'a'..'z', '0'..'9', '_']) do
  begin
    FirstWord := FirstWord + UpCase(S[I]);
    Inc(I);
  end;
  if ((FirstWord = 'FOR') or (FirstWord = 'WHILE') or (FirstWord = 'WITH'))
     and not HasWord('DO') then Exit;
  if (FirstWord = 'IF') and not HasWord('THEN') then Exit;
  if (FirstWord = 'CASE') and not HasWord('OF') then Exit;
...
function IsHandledDiagCode(const ACode: string): Boolean;
const
  Handled: array[0..13] of string = (
    'E2003', 'F2613', 'F2063', 'E2037', 'H2443', 'H2164',
    'E2029', 'E2066', 'W1036', 'H2077', 'W1010', 'E2065', 'H2219', 'E2291');
begin
  Result := False;
        ...
        try
          ...
          try
            ...
            begin
              ...
              // Only a push for THIS analysis describes AContent; after a
              // timeout the stored list may belong to an older state.
              if Client.GetFileDiagnosticsVersion(AFile) > BeforeVer then
                StoreDiagnostics(AFile, SrcLsp, AContent, Diags);
...
begin
  ...
  begin
    ...
    begin
      ...
      // A NEW appearance restarts the clock: a different window, or the
      // same handle after a gap in which no probe saw it (handles are
      // recycled, and the IDE shows and hides the SAME completion window
      // again and again - it must not count as one long-lived blocker).
      if (Blocker <> FBlockWnd) or (GetTickCount - FBlockLastSeen > 2500) then
      begin
        FBlockWnd := Blocker;
        FBlockSince := GetTickCount;
        FBlockCls := BlockCls;
      end;
      FBlockLastSeen := GetTickCount;
...
begin
  ...
  while (TInterlocked.CompareExchange(GLiveWorkers, 0, 0) <> 0)
    and (Waited < 35000) do   // above the worker's own worst case (~27 s)
  begin
    CheckSynchronize(10);
    Inc(Waited, 10);
  end;
...
procedure LiveReportErrorDiags(const AFile, AContent: string;
  const ADiags: TArray<TLspErrorDiag>; const ASource: string);
begin
  // The raw diagnostics per source, bound to the content they describe -
  // what the MCP bridge reports (the live checker keeps only the fixes).
  StoreDiagnostics(AFile, ASource, AContent, ADiags);
  if GLive <> nil then
